...
function part_file_upload(req, res) {
	async function handle_file() {
		try {
			...
			const { size, buffer, originalname, mimetype } = req.file;

			if (size >= 15 * 1000 * 1000) {
				throw new CustomError('Tamanho do arquivo excedeu o limite: 15MB', 400);
			}
			...
			const part_volumes_promise = get_part_volume(buffer, originalname, mimetype);
	...
	upload.single('file')(req, res, handle_file);
}

function aux_file_upload(req, res) {
	async function handle_file() {
		try {
